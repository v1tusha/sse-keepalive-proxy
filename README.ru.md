...
2. **Авто-ретрай** — транзиентные ошибки шлюза (401/403/429/5xx, например
   ...
   повторяет сам с линейным бэкоффом (`RETRY_DELAY_MS`, потом вдвое больше, и
   так далее), и Claude Code вообще не видит ошибку. Если шлюз присылает
   `retry-after` — слушаем его, а не свой график (с потолком 30с, чтобы шлюз не
   мог припарковать запрос).
...
Оба эндпоинта отклоняют любой запрос с заголовком `Origin`. Они меняют живой
конфиг, а `/__config` разбирает тело с любым content-type — то есть это CORS
«simple request»: без этой проверки любая открытая в браузере страница могла бы
сделать POST на `127.0.0.1:8787` и переключить ремап или выставить `hedgeMs` в
режим лавины. Пульт и `curl` `Origin` не посылают, их это не касается.
...
| `RETRY_DELAY_MS` | `1500` | Базовая пауза перед повтором после транзиентной ошибки; растёт с каждой попыткой, а `retry-after` от шлюза её перебивает |
...
stream closed normally: 12446b +2 keepalive in 13840ms  поток дошёл до конца
