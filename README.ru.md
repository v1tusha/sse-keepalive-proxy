...
3. **Пре-коммит** — keepalive можно инжектить только после того, как шлюз отдал
   заголовки ответа. Если он молчит дольше `PRE_COMMIT_MS`, прокси отдаёт
   SSE-заголовки сам и начинает капать keepalive, а попытки продолжаются за
...
POST /v1/messages keepalive #2                   вставлен keepalive
